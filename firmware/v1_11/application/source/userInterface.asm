...
;**********************************************************************
; Function: void userInterface_checkConfigRequest(void)
;**********************************************************************
userInterface_checkConfigRequest
	PUSH_R	r0
	
	; read buttons (buttons are active-low so reverse logic)
	comf	PORTC, w, ACCESS
	andlw	0x07
	movwf	r0

	; if waveform (RC2) is pressed then do midiSelectChannel()
	movlw	1<<RC2
	cpfseq	r0, ACCESS
	bra		userInterface_checkConfig_notWF
	call	midiSelectChannel
	bra		userInterface_checkConfig_exit
userInterface_checkConfig_notWF

;	; if record (RC1) is pressed then do ?()
;	movlw	1<<RC1
;	cpfseq	r0, ACCESS
;	bra		userInterface_checkConfig_notREC
;	call	?
;	bra		userInterface_checkConfig_exit
;userInterface_checkConfig_notREC

;	; if mode (RC0) is pressed then do ?()
;	movlw	1<<RC2
;	cpfseq	r0, ACCESS
;	bra		userInterface_checkConfig_notMODE
;	call	?
;	bra		userInterface_checkConfig_exit
;userInterface_checkConfig_notMODE

userInterface_checkConfig_exit
