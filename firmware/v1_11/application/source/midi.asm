...
;**********************************************************************
; Function: void midiSelectChannel()
;**********************************************************************
midiSelectChannel
	PUSH_R	r0
	
	; configure LEDs to display current MIDI channel
midiSelectChannel_refreshLEDs
	; increment value so that midi channel is logical to user and display is not blank
	incf	midiChannel, w, ACCESS
	; preset to all off
	LED_ALL_OFF
	; roll bit 0 out of channel, turn on SAMPLE LED if set
	rrcf	WREG, f, ACCESS
	btfsc	STATUS, C, ACCESS
	LED_SAMPLE_ON
	; roll bit 1 out of channel, turn on SQUARE LED if set
	rrcf	WREG, f, ACCESS
	btfsc	STATUS, C, ACCESS
	LED_SQUARE_ON
	; roll bit 2 out of channel, turn on SAMPLE SINE if set
	rrcf	WREG, f, ACCESS
	btfsc	STATUS, C, ACCESS
	LED_SINE_ON
	
	; wait for button press
midiChannelSelect_waitForPress
	comf	PORTC, w, ACCESS
	andlw	0x07
	movwf	r0, ACCESS
	bz	midiChannelSelect_waitForPress

	; button pressed

	; if button == MODE then save channel setting to EEPROM and exit
	movlw	1<<RC0
	cpfseq	r0, ACCESS
	bra		midiChannelSelect_notMOD
	; wait for button to release
midiChannelSelect_waitReleaseMOD
	comf	PORTC, w, ACCESS
	btfsc	WREG, RC0, ACCESS
	bra		midiChannelSelect_waitReleaseMOD
	; write midiChannel to EEPROM and exit
	WRITE_INTERNAL_EEPROM	2, midiChannel
;	bra 	midiChannelSelect_exit
	reset
midiChannelSelect_notMOD		

	; if button == RECORD then increment channel
	movlw	1<<RC1
	cpfseq	r0, ACCESS
	bra		midiChannelSelect_waitForPress
	; wait for button to release
midiChannelSelect_waitReleaseREC	
	comf	PORTC, w, ACCESS
	btfsc	WREG, RC1, ACCESS
	bra		midiChannelSelect_waitReleaseREC
	; increment midiChannel
	incf	midiChannel, f, ACCESS
	movlw	0x07
	xorwf	midiChannel, w, ACCESS
	bnz		midiSelectChannel_refreshLEDs
	clrf	midiChannel, ACCESS
	; refresh LEDs to indicate new channel value
	bra		midiSelectChannel_refreshLEDs
	
midiChannelSelect_exit
	POP_R	r0
	return
...
initMIDI
	...
	; recall saved midiChannel from uC's internal EEPROM
	movlw	2
	call	eepromInternalRead
	movwf	midiChannel, ACCESS	
...
midiMessageMapper
	...
	; if statusByte != 0xf0 then verify correct channel
	xorlw	0xf0
	bz	midiMessageMapper_skipChannelCheck
	; message is not SysEx
	; ignore incoming channel if internal midiChannel == 6 (MIDI channel 7)
	movf	midiChannel, w, ACCESS
	xorlw	6
	bz		midiMessageMapper_skipChannelCheck
	; verify correct channel, ignore if ((midiRxMessage[0] & 0x0f) != midiChannel)
	movlw	0
	movf	PLUSW_midiRxMessage, w, ACCESS
	; mask out channel
	andlw	0x0f
	; compare channel
	cpfseq midiChannel, ACCESS
	goto	midiMessageMapper_exit
midiMessageMapper_skipChannelCheck
